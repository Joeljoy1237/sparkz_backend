...
router.get('/eventResgistrations/:eventId', async (req, res) => {
  try {
    ...
    const eventResgistrations = await Register.find({ event: eventId }).populate('event registeredUser', 'email firstName mobNo dob class college semester department title department isTeam');
